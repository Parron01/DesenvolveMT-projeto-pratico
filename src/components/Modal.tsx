...
export function Modal({ title, description, isOpen, onClose, children }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50" aria-modal="true" role="dialog" aria-labelledby="modal-title" aria-describedby={description ? "modal-desc" : undefined}>
      ...
      <div className="absolute inset-0 flex items-start sm:items-center justify-center p-2 sm:p-3 lg:p-4">
        <div ref={dialogRef} className="w-full max-w-4xl xl:max-w-5xl 2xl:max-w-6xl rounded-2xl bg-white shadow-2xl outline-none" tabIndex={-1}>
          <div className="flex items-start justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-neutral-100">
            <div className="pr-4">
              <h2 id="modal-title" className="text-base sm:text-lg lg:text-xl font-bold text-neutral-900 leading-tight">{title}</h2>
          ...
          <div className="max-h-[80vh] sm:max-h-[75vh] overflow-y-auto px-4 sm:px-6 py-4 sm:py-6">{children}</div>
        </div>
      </div>
    </div>
  )
